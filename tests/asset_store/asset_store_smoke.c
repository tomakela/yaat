...
static void expect_read(const char *name, YaatAssetStore *store,
                        const char *path, const char *want)
{
    YaatAssetReadResult result;
    yaat_asset_store_read(store, path, &result);
...
static void expect_missing(YaatAssetStore *store)
{
    YaatAssetReadResult result;
    yaat_asset_store_read(store, "missing.txt", &result);
    expect_true("missing asset fails clearly", !result.ok && strstr(result.error, "Missing asset") != 0);
}

static void expect_unsafe(YaatAssetStore *store, const char *path)
{
    YaatAssetReadResult result;
    yaat_asset_store_read(store, path, &result);
...
int main(void)
{
    ...
    yaat_asset_store_init(&store, "tests/fixtures/assets");
    ...
    yaat_asset_store_init(&store, "tests/fixtures/assets_patched");
    expect_read("patch0001 overrides the same file again", &store, "data/value.txt", "patch one\n");
    yaat_asset_store_init(&store, "tests/fixtures/assets_patch0");
    ...
    expect_true("mixed slashes normalize safely",
                yaat_asset_normalize_path("safe\\mixed/path.txt", normalized, sizeof(normalized)) &&
                strcmp(normalized, "safe/mixed/path.txt") == 0);
    expect_true("mixed slash traversal is rejected",
                !yaat_asset_normalize_path("safe\\..\\x", normalized, sizeof(normalized)));

    yaat_asset_store_init(&store, "tests/fixtures/assets_duplicate");
