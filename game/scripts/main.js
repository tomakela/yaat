...
function update(){ const dx=state.tx-state.px, dy=state.ty-state.py, d=Math.hypot(dx,dy); if(d>0){ const step=Math.min(SPEED,d); state.px+=dx/d*step; state.py+=dy/d*step; if(dx<0) state.face=0; else if(dx>0) state.face=1; updateSuppressedExit(); const exit=playerExit(); if(exit){ changeRoomFrom(exit); return; } } if(d<=SPEED && state.pending){ const p=state.pending; state.pending=null; interact(p); } else if(d<=SPEED&&!state.pending){ if(state.actionSentence&&!state.actionUntil) clearActionSentence(); updateSuppressedExit(); const exit=playerExit(); if(exit) changeRoomFrom(exit); } }
