...
function findEntityEvent(o, verb, itemId, defaultVerb){
  let ev;
  if(itemId) ev=o?.events?.find(e=>e.name===verb && e.item===itemId);
  if(!ev) ev=o?.events?.find(e=>e.name===verb && !e.item);
  const fallback=defaultVerb||verb;
  if(!ev && fallback!==verb) ev=o?.events?.find(e=>e.name===fallback && !e.item);
  return ev;
}
function runEntityVerb(o, verb, itemId, defaultVerb){
  const ev=findEntityEvent(o, verb, itemId, defaultVerb);
  if(ev){ runScriptCommands(ev.commands,{target:o}); return true; }
  return false;
}
function activeVerbEvent(o,verb){ return (verb==='use'&&state.selectedInv) ? findEntityEvent(o,'use',state.selectedInv) : findEntityEvent(o,verb,null); }
function hasVerbAction(o,verb){ return !!(o&&(verb==='walk'||activeVerbEvent(o,verb)||(verb==='use'&&!state.selectedInv&&o.inventoryItem))); }
function shouldWalkForVerb(o,verb){ if(!state.playerVisible) return false; if(!hasVerbAction(o,verb)) return false; if(verb==='look') return false; const event=activeVerbEvent(o,verb); return event ? event.walkBefore!==false : true; }
...
function beginPendingInteraction(o){ walkToObj(o); if(state.verb==='walk'&&!state.selectedInv&&canChangeRoom(o)) state.suppressedExit=o.id; state.pending=state.verb==='walk'&&!state.selectedInv&&o.inventoryItem?null:o; }
function queueChainedGroundUse(o){ if(!(state.pending?.inventoryItem&&state.verb==='use'&&!state.selectedInv&&o!==state.pending)) return false; state.chained=o; const itemName=inventoryDefs[state.pending.inventoryItem]?.name||state.pending.name; setActionSentence(`Use ${itemName} with ${o.name}`); return true; }
...
function interact(o){ if(!o) return; const verb=state.selectedInv?'use':state.verb; if(verb==='walk'){ if(canChangeRoom(o)) changeRoomFrom(o); else runEntityVerb(o,'walk',null,null); clearActionSentence(); return; } if(verb==='use'&&!state.selectedInv&&o.inventoryItem){ if(!runEntityVerb(o,'take',null,null)) cant(verb); if(state.inv.includes(o.inventoryItem)){ state.verb='use'; state.selectedInv=o.inventoryItem; if(continueChainedInteraction()) return; } clearActionSentence(); return; } if(!runEntityVerb(o,verb,verb==='use'?state.selectedInv:null,null)) cant(verb); if(state.selectedInv) state.suppressedExit=o.id; deselectVerb(); clearActionSentence(); }
function triggerCursorLongClick(){ if(performance.now()<state.splashUntil){ state.splashUntil=0; return; } const x=state.cx,y=state.cy; if(y>=PLAY_H){ const ii=state.inv.findIndex((_,i)=>x>=164+i*23&&x<184+i*23&&y>=PLAY_H+4&&y<PLAY_H+24); if(ii>=0){ const def=inventoryDefs[state.inv[ii]]; setActionSentence(`Look at ${def?.name||state.inv[ii]}`,1000); say('player',def?.description||`I have ${def?.name||state.inv[ii]}.`); } return; } const o=at(x,y); if(o){ setActionSentence(`Look at ${o.name}`,1000); if(!runEntityVerb(o,'longclick',null,'look')) cant('look'); } }
function update(now=performance.now()){ const elapsed=state.lastUpdate?Math.min(now-state.lastUpdate,MAX_WALK_DELTA_MS):1000/30; state.lastUpdate=now; const maxStep=PLAYER_WALK_SPEED*elapsed/1000; const dx=state.tx-state.px, dy=state.ty-state.py, d=Math.hypot(dx,dy); if(d>0){ const step=Math.min(maxStep,d); state.px+=dx/d*step; state.py+=dy/d*step; if(dx<0) state.face=0; else if(dx>0) state.face=1; updateSuppressedExit(); if(!state.pending){ const exit=playerExit(); if(exit){ changeRoomFrom(exit); return; } } } if(d<=maxStep && state.pending){ const p=state.pending; state.pending=null; interact(p); } else if(d<=maxStep&&!state.pending){ if(state.actionSentence&&!state.actionUntil) clearActionSentence(); updateSuppressedExit(); const exit=playerExit(); if(exit) changeRoomFrom(exit); } }
...
function triggerCursorClick(){ if(performance.now()<state.splashUntil){ state.splashUntil=0; return; } if(state.dialog||state.sayQueue.length){ advanceSay(); return; } const x=state.cx,y=state.cy; if(y>=PLAY_H){ if(state.playerVisible===false) return; const vi=verbs.findIndex((_,i)=> x>=4+(i%3)*50&&x<52+(i%3)*50&&y>=PLAY_H+3+(i/3|0)*15&&y<PLAY_H+16+(i/3|0)*15); if(vi>=0){ state.verb=verbs[vi]; state.selectedInv=''; return; } const ii=state.inv.findIndex((_,i)=>x>=164+i*23&&x<184+i*23&&y>=PLAY_H+4&&y<PLAY_H+24); if(ii>=0){ const id=state.inv[ii], def=inventoryDefs[id]; if(state.verb==='look'){ setActionSentence(`Look at ${def?.name||id}`,1000); say('player',def?.description||`I have ${def?.name||id}.`); deselectVerb(); } else if(state.verb==='use'){ state.selectedInv=id; } else { setActionSentence(`${verbLabels[state.verb]||state.verb} ${def?.name||id}`,1000); cant(state.verb); deselectVerb(); } return; } return; } const o=at(x,y); if(o){ if(!state.playerVisible){ if(!runEntityVerb(o,'click',null,null)) cant('click'); return; } const verb=state.selectedInv?'use':state.verb; if(queueChainedGroundUse(o)) return; const command=state.selectedInv?`Use ${inventoryDefs[state.selectedInv].name} with `:(verbLabels[state.verb]||state.verb)+' '; setActionSentence(command+o.name); if(!hasVerbAction(o,verb)||!shouldWalkForVerb(o,verb)){ interact(o); } else { beginPendingInteraction(o); } } else { clearActionSentence(); state.pending=null; state.chained=null; if(state.selectedInv||state.verb!=='walk'){ deselectVerb(); return; } state.tx=clamp(x,PLAYER_W/2,W-PLAYER_W/2); state.ty=clamp(y,PLAYER_H,PLAY_H-1); } }
