...
function interact(o){ if(!o) return; const verb=state.selectedInv?'use':state.verb; if(verb==='walk'){ if(!canChangeRoom(o)&&o.use) o.use.call(o); clearActionSentence(); return; } if(verb==='use'&&!state.selectedInv&&o.inventoryItem){ if(!(o.click&&o.click.call(o))){ if(o.take) o.take.call(o); else cant(verb); } if(state.inv.includes(o.inventoryItem)){ state.verb='use'; state.selectedInv=o.inventoryItem; if(continueChainedInteraction()) return; } clearActionSentence(); return; } if(o[verb]) o[verb].call(o); else cant(verb); if(state.selectedInv) state.suppressedExit=o.id; deselectVerb(); clearActionSentence(); }
