...
'use strict';
const W=320,H=240,PLAY_H=200,PLAYER_W=18,PLAYER_H=34,PLAYER_WALK_SPEED=60,MAX_WALK_DELTA_MS=50,SPLASH_MS=2000,DIALOG_MS=2400,TOUCH_CURSOR_SENSITIVITY=1.75,TOUCH_DRAG_THRESHOLD=3,TOUCH_LONGPRESS_MOVE_THRESHOLD=12,TOUCH_LONGPRESS_MS=550;
...
const state={playerVisible:true,splashUntil:performance.now()+SPLASH_MS,room:'room000_start',px:160,py:100,tx:160,ty:100,face:1,cx:160,cy:100,verb:'walk',selectedInv:'',inv:[],vars:{...initialVars},dialog:null,dialogUntil:0,sayQueue:[],actionSentence:'',actionUntil:0,shakeUntil:0,shakeMag:0,pending:null,suppressedExit:null,lastUpdate:0};
...
function interact(o){ if(!o) return; const verb=state.selectedInv?'use':state.verb; if(verb==='walk'){ if(canChangeRoom(o)) changeRoomFrom(o); else if(o.use) o.use.call(o); clearActionSentence(); return; } if(verb==='use'&&!state.selectedInv&&o.inventoryItem){ if(o.click) o.click.call(o); else if(o.take) o.take.call(o); else cant(verb); clearActionSentence(); if(state.inv.includes(o.inventoryItem)){ state.verb='use'; state.selectedInv=o.inventoryItem; } return; } if(o[verb]) o[verb].call(o); else cant(verb); if(state.selectedInv) state.suppressedExit=o.id; deselectVerb(); clearActionSentence(); }
...
function drawBmp(path,x,y,w,h,transparentColor=''){ const im=img(path); if(imageLoaded(im)){ const drawable=transparentImg(path,transparentColor); ctx.drawImage(drawable,x,y,w||im.naturalWidth,h||im.naturalHeight); } else if(im.failed) rect(x,y,w||24,h||24,hashColor(path,0x2f5f9e)); }
...
function drawPlayer(sx,sy){ const moving=Math.abs(state.px-state.tx)+Math.abs(state.py-state.ty)>0.1; const path=playerSpritePath(moving); const im=img(path); if(imageLoaded(im)) ctx.drawImage(transparentImg(path,playerTransparentColor),state.px-im.naturalWidth/2+sx,state.py-im.naturalHeight+sy); else { rect(state.px-11+sx,state.py+7+sy,22,5,'#664f38'); rect(state.px-5+sx,state.py-34+sy,10,10,'#5a3a24'); rect(state.px-6+sx,state.py-25+sy,12,15,'#2f5f9e'); rect(state.px-9+sx,state.py-22+sy,4,12,'#274774'); rect(state.px+5+sx,state.py-22+sy,4,12,'#274774'); rect(state.px-5+sx,state.py-10+sy,4,10,'#1f2430'); rect(state.px+1+sx,state.py-10+sy,4,10,'#1f2430'); } }
