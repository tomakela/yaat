...
const state=new GameState(pkg,{currentRoom:'demo'}); const vm=new ScriptVm(pkg,state); let effects=vm.runEvent(pkg.rooms[0].events[0]); let dialog=effects.find(e=>e.type==='say');
...
canvas.addEventListener('click',()=>{ effects=vm.runEvent(pkg.rooms[0].events[0]); dialog=effects.find(e=>e.type==='say') || dialog; draw(); });
draw();
