                        ...
                        YaatAssetReadResult *result)
{
    ...
    int found = 0;
    if (fp == 0) return 0;
    if (fgets(line, sizeof(line), fp) == 0 || strncmp(line, YAAT_ARCHIVE_MAGIC, 7) != 0) {
        fclose(fp);
        return 0;
    }
    while (fgets(line, sizeof(line), fp) != 0) {
        ...
        *colon = '\0';
        ...
        if (yaat_asset_normalize_path(entry_path, normalized, sizeof(normalized)) && strcmp(normalized, logical) == 0) {
            result->size = (int)fread(result->data, 1, (size_t)size, fp);
            result->data[result->size] = '\0';
            result->ok = 1;
            result->error[0] = '\0';
        ...
        fgetc(fp); /* trailing newline after payload */
    }
    fclose(fp);
    return found;
}
